...
def post_check(thread: str, standing_goals: list[dict], after_live: list[int],
    ...
    пушем и письмом-тревогой и остаётся в снимке направления.
    """
    ...
    if announce:
        ...
        thread_tick.deliver(thread, "alarm",
                            "Продакт: пробуждение по стоячей цели кончилось молчанием",
                            told, None, moment)
    return {
        "at": moment.isoformat(),
        "resolved": False,
...
сделай следующий безопасный шаг сам. Ответом верни короткий текст для
пользователя в формате вердикта продакта либо ровно слово SILENT, если пользователю
сказать нечего. Первой строкой `ПОВОД: вопрос|польза|готово|механика`, второй
`ВОПРОС: да|нет`.
"""
...
Действуй сам. Ответом верни короткий текст для пользователя в формате вердикта
продакта либо ровно слово SILENT. Первой строкой `ПОВОД: вопрос|польза|готово|механика`,
второй `ВОПРОС: да|нет`.
"""
...
Продолжай с того места, где остановился. Ответом верни короткий текст для
пользователя в формате вердикта продакта либо ровно слово SILENT. Первой строкой
`ПОВОД: вопрос|польза|готово|механика`, второй `ВОПРОС: да|нет`.
"""


def _rules_block() -> str:
    """Правила, одинаковые на всех ходах: одно место, чтобы они не разъехались."""
    return """Правила этого режима:
...
- значимое изменение снимка продукта пиши через
  `python3 /opt/projects/product-owner/scripts/product_memory.py`, а не правкой
  файла руками."""
