...
def written_at(path: Path) -> str:
    """When the file carrying the observation was last written, or now.

    A file that vanished between the look and the `stat` leaves nothing to read
    the instant from; then, and only then, the time of the look is all there is.
    """
    try:
...
class Scribe:
    ...
    def observe_task(self, task_dir: Path) -> None:
        ...
        # When the change was written, not when the look reached it. The scribe
        # is not a daemon: it is called while the board is open, so a board
        # nobody opened for a week leaves a gap, and the first look after it
        # finds every transition of that week at once. Stamping those with the
        # time of the look turned week-old completions into today's news and
        # pushed genuinely fresh records out of the six the feed shows (finding
        # HIGH-1 of the independent review of task 1067). `task.md` is the file
        # this status is read from, so the instant it was last written is the
        # observed instant of the change — the same source the appearance record
        # already trusts. It can stand later than the transition when the file
        # was rewritten again afterwards; what it can never be is a moment the
        # file was not written at, which is exactly what the time of the look
        # was.
        stamp = written_at(task_dir / "task.md")
        if name not in self.cursor["status"]:
            started = state.read_json(task_dir / ".runner" / "runner.json").get("started_at")
            self.emit({**base, "at": started or stamp,
                       "kind": "task_appeared",
                       "label": title, "observed_by": "каталог задачи на диске"})
        elif self.cursor["status"][name] != front["status"]:
            self.emit({**base, "at": stamp, "kind": "task_status",
                       "label": f"{self.cursor['status'][name]} → {front['status']}",
                       "observed_by": "frontmatter task.md (время по mtime файла)",
                       "status": front["status"]})
