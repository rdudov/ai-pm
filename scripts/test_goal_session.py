...
class SignificantChange(unittest.TestCase):
    ...
    def test_a_new_commit_in_the_directions_tree_is_a_change(self):
        said = session.changes(look(heads={"/opt/projects/companion-agent": "aaa"}),
                               look(heads={"/opt/projects/companion-agent": "bbb"}))
        self.assertEqual(said, ["companion-agent: новый коммит bbb"])
...
class StandingGoalOutcome(unittest.TestCase):
    ...
    def test_silence_beside_an_unrelated_live_run_is_refused(self):
...
class TheTickBecomesAWatchdog(unittest.TestCase):
    ...
    def test_the_outcome_line_tells_a_held_direction_from_a_quiet_one(self):
        ...
        self.assertIn("непрерывная сессия s-1", held)
