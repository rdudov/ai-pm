...
SECTION_PROMPTS = {
    ...
    "В работе": (
        "Только текущие обещания с наблюдаемой задачей; события и прежние "
        "состояния пишутся в history/. Новое обещание добавляется через "
        "`product_memory.append_work_line`, а не руками: рядом работает второй "
        "продакт."),
    "Журнал эффекта": (
