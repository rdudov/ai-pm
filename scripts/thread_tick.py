...
FAILED_SEND = {
    ...
    "task_statement": "обязательное письмо с постановкой не ушло; задача не стартует",
    "task_completion": "обязательное письмо с результатом не ушло; задача не закрывается",
}
...
- любое письмо самодостаточно: его читают, не открывая задач и не помня
  ...
  каждой номер задачи и одна фраза, что это и в какой стадии. Дальше отдельно
  и прямо скажи, что уже изменилось для пользователя сейчас. Если принятый код
  ещё не установлен или опыт только предстоит, напиши, что в работающем
  продукте пока ничего не изменилось, а будущую установку или проверку назови
  отдельно. Затем назови цену, что осталось и «Риск/долг», если в verification
  есть GAP;
...
def main() -> int:
    ...
    # возвращённого внешнего результата в той же задаче. Уходит один раз на
