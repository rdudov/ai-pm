...
def notify(text: str) -> None:
    ...
    прогонах приходят своим транспортом из task-system, эта правка их не
    касается.
    """
...
def verdict_block() -> str:
    ...
    `message-id`) was five sentences with no heading, no task number and
    ...
    """
    return """Правила ответа пользователю:
...
- пиши простым инженерным русским. Подлежащее, сказуемое, прямой порядок слов.
  Активный залог: «продакт удалил», а не «деревья удалены». Одно утверждение в
  предложении. Предложение — не длиннее примерно двадцати слов, абзац — не
  длиннее четырёх предложений. Не пиши вводных оборотов и книжных формул
  («по слову пользователя», «наблюдённый повод», «этим не отменяется»,
  ...
  подменяй причину метафорой: «дверь», «стена», «закапывает». Пользователь
  сказал 23 августа: «мне реально сложно читать, что ты пишешь… я трачу больше
  времени и быстрее устаю»;
