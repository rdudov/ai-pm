...
- обычный вердикт остаётся коротким; если сказать пользователю нечего, ответь
  ровно словом `SILENT`;
...
- человек должен суметь принять решение из одного этого письма, не зная номеров
  задач и предыдущего разговора. Подробность вопроса важнее требования
  краткости, но не повторяй сведения, которые не помогают выбору. Не выдумывай
...
- первой строкой поставь `ПОВОД: вопрос|польза|готово|механика`, второй —
  ...
  письмо доходит всегда. `польза` — изменилось, что пользователь может;
  `готово` — закончилась заказанная работа; `механика` — прогон или движение
  репозитория, которые идут пушем и на табло. Просьба выбрать, даже кончающаяся
  точкой, требует `ВОПРОС: да`."""
